...
type typ = NamedType of string | ListType of typ | NonNullType of typ
...
module Pp = struct
  ...
  let rec pp_value fmt = function
    ...
    | `Assoc props ->
        Fmt.(braces (list ~sep:comma (pair ~sep:colon string pp_value)))
          fmt props

  let omit_empty_list t fmt = function [] -> () | xs -> t fmt xs

  let arguments fmt args =
    omit_empty_list
      Fmt.(parens (list ~sep:comma (pair ~sep:colon string pp_value)))
      fmt args
  ...
  let pp_fragment_spread fmt (fragment_spread : fragment_spread) =
    Fmt.fmt "...%s%a" fmt fragment_spread.name directives
  ...
  let rec pp_selection fmt = function
    | Field f -> (
        match f.alias with
        | Some alias ->
            Fmt.fmt "%s: %s%a%a%a" fmt alias f.name arguments f.arguments
    ...
    | InlineFragment f -> (
        ...
        | Some condition ->
            Fmt.fmt "... on %s %a %a" fmt condition directives f.directives
  ...
  let pp_variable_definition fmt var_def =
    ...
    | None -> Fmt.fmt "$%s : %a" fmt var_def.name pp_typ var_def.typ
    | Some value ->
        Fmt.fmt "$%s : %a = %a" fmt var_def.name pp_typ var_def.typ pp_value
          value

  let variables =
    omit_empty_list Fmt.(parens (list ~sep:comma pp_variable_definition))
  ...
  let pp_operation fmt op =
    match op.name with
    | None -> selection_set fmt op.selection_set
    | Some name ->
        Fmt.fmt "%a %s%a%a %a" fmt pp_optype op.optype name variables
          op.variable_definitions directives op.directives selection_set
          op.selection_set

  let pp_fragment fmt (f : fragment) =
    Fmt.fmt "fragment %s on %s %a %a" fmt f.name f.type_condition directives
      f.directives selection_set f.selection_set
