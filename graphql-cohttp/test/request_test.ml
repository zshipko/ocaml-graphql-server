    ...
    schema
      [
        field "hello" ~typ:(non_null string)
          ...
          ~resolve:(fun _ () -> function None -> "world" | Some name -> name);
      ])
...
let check_response_action ~rsp ~rsp_body actual =
  match actual with
  | `Expert _ -> Alcotest.fail "Got expert, expected response"
  | `Response (rsp', body') ->
  ...
  [
    ...
    ( "POST with json body",
      ...
      fun () ->
        ...
        test_case
          ~req:
            (Cohttp.Request.make ~meth:`POST ~headers:json_content_type
               default_uri)
          ~req_body
    ...
    ( "POST with graphql body",
      ...
      fun () ->
        ...
        test_case
          ~req:
            (Cohttp.Request.make ~meth:`POST ~headers:graphql_content_type
               default_uri)
          ~req_body
          ~rsp:(Cohttp.Response.make ~status:`OK ())
          ~rsp_body:default_response_body );
            ...
            (Yojson.Basic.to_string
               (`Assoc
                 [
                   ( "query",
                     `String
                       "query A { hello(name: \"world\") } query B { \
                        hello(name: \"fail\") }" );
