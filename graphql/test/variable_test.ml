  ...
  [
      ...
      fun () ->
        ...
        test_query variables query
          (`Assoc
            [
              ( "data",
                `Assoc [ ("bool_list", `List [ `Bool false; `Bool true ]) ] );
            ]) );
    ( "input object variable",
      ...
      fun () ->
        ...
        let query =
          "{ input_obj(x: {title: \"Mr\", first_name: \"John\", last_name: \
           \"Doe\"}) }"
        in
        test_query variables query
    ...
    ( "variable coercion: string to ID",
      `Quick,
      fun () ->
    ...
    ( "variable overrides default variable",
      `Quick,
      fun () ->
        let variables = [ ("x", `Int 43) ] in
        let query = "query has_defaults($x : Int! = 42) { int(x: $x) }" in
        test_query variables query
          (`Assoc [ ("data", `Assoc [ ("int", `Int 43) ]) ]) );
  ]
